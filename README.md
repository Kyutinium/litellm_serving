...
### 요청 로깅 (`request_logger.py`)

JSON Lines 형식으로 요청 1건 = 1줄씩 파일에 기록합니다. **`LITELLM_LOG_FILE`을 비워두면 완전히 비활성화**되어 오버헤드가 없습니다.

| 변수 | 설명 | 기본값 |
|---|---|---|
| `LITELLM_LOG_FILE` | 로그 파일 경로. **비어 있으면 로깅 OFF** | *(없음 = OFF)* |
| `LITELLM_LOG_LEVEL` | `info` (메타데이터만) / `debug` (메시지·응답 본문 포함) | `info` |
| `LITELLM_LOG_MAX_BYTES` | 파일 1개당 최대 바이트(회전 기준) | `52428800` (50 MB) |
| `LITELLM_LOG_BACKUP_COUNT` | 보관할 회전 파일 개수 | `5` |

기록되는 필드: `ts`, `event`(`success`/`failure`), `model`, `user`(api key의 SHA-256 16자), `request_id`, `call_type`, `stream`, `latency_ms`, `usage`, `error`(실패 시). `debug`에선 `messages`, `response_text`까지 포함.

```bash
# Docker — 로그를 호스트로 영구화하려면 docker-compose.yml의 logs 볼륨/ENV 주석 해제

# Docker 미사용
export LITELLM_LOG_FILE=./logs/requests.jsonl
...
├── request_logger.py      # 선택적 요청/응답 JSON Lines 로깅 (LITELLM_LOG_FILE)
├── Dockerfile
├── docker-compose.yml
