...
class StripThinkingPatchTests(unittest.TestCase):
    def _load_strip_thinking(
        self, mode, original_result, enabled="true", patch_streaming=True
    ):
        ...
        os.environ["STRIP_THINKING_ENABLED"] = enabled
    ...
    def test_disabled_apply_patch_bypasses_callback_and_streaming_patch(self):
        adapter_cls = self._load_strip_thinking(
            "reasoning_fallback",
            ("thinking_delta", SimpleNamespace(thinking="visible answer")),
            enabled="false",
            patch_streaming=False,
        )
        strip_thinking = sys.modules["strip_thinking"]
        litellm = sys.modules["litellm"]

        strip_thinking.apply_patch()
        type_of_content, delta = (
            adapter_cls()._translate_streaming_openai_chunk_to_anthropic([])
        )
    ...
    def test_reasoning_fallback_mode_promotes_reasoning_only_thinking_delta(self):
        adapter_cls = self._load_strip_thinking(
            "reasoning_fallback",
            ("thinking_delta", SimpleNamespace(thinking="visible answer")),
        )
    ...
    def test_none_mode_drops_thinking_delta_to_avoid_normal_behavior_change(self):
        adapter_cls = self._load_strip_thinking(
            "none", ("thinking_delta", SimpleNamespace(thinking="hidden thinking"))
        )
    ...
    def test_input_stripping_preserves_tool_result_blocks(self):
        self._load_strip_thinking(
        ...
        messages = [
            ...
            },
            {
                "role": "assistant",
                "content": [
                    {"type": "thinking", "thinking": "hide me"},
                    {"type": "text", "text": "visible"},
                ],
            },
        ]

        cleaned = strip_thinking._strip_thinking_from_messages(messages)
        ...
        self.assertEqual(cleaned[1]["content"], "visible")
